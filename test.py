...
class SchnapsenActionRepresentation:
    def __init__(self):
        """
...
# Usage example
action_rep = SchnapsenActionRepresentation()

# Example of encoding a move
card = Card.get_card(Rank.JACK, Suit.HEARTS)
regular_move = RegularMove(card)
action_id = action_rep.encode_action(regular_move)
print(f"Encoded action ID for {regular_move}: {action_id}")

# Example of decoding an action
decoded_move = action_rep.decode_action(action_id)
print(f"Decoded move from ID {action_id}: {decoded_move}")

# Example of creating an action tensor
valid_moves = [regular_move]
action_tensor = action_rep.set_valid_actions(valid_moves)
print("Action tensor with valid moves set:", action_tensor)
