...
The unused Python agent files described here have been removed from the working tree and remain available in Git history. This is a historical consumer audit, not the current API or the rules specification.

* The historical `main.py` created `SchnapsenGamePlayEngine`, calls `play_game(bot1, bot2, Random())`, and consumes `(winner_bot, game_points, winner_score)`. Evaluation plays 100 games against `RandBot`, with the tested bot always initially leading, and counts bot identity wins. This is policy evaluation, not a compatibility test.
...
Rust `PlayerObservation` covers those observable data through owned values. It does not expose privileged state pointers. Python's `get_engine`, `make_assumption`, `get_state_in_phase_two` and bot-search convenience wrappers are not reproduced as a Python API; consumers can build search/determinization separately. The current `native/` Python extension implements the Rust batch environment; it does not recreate the legacy runtime bot callback API.
